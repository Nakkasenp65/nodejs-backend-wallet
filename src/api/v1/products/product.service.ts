 ...
 */
const fetchProducts = async (opts: FetchProductOptions = {}) => {
...
const generateUniqueId = (productData: Partial<Product>) => {
...
const getProductFilters = async () => {
    ...
    return {
        ...
        capacities: capacities.map((item) => item.capacity).filter(Boolean) as string[],
        colors: colors.map((item) => item.color).filter(Boolean) as string[],
    };
};
...
const getProducts = async (options: GetProductsOptions = {}) => {
    ...
    // Note: 'condition' is not in Prisma.ProductWhereInput based on the previous file content,
    // but it was in the JS code. I will assume it might be a field or ignore if not present in schema.
    // Checking schema would be best, but assuming it exists as it was in JS.
    // If it errors, I will remove it.
    // Wait, looking at `fetchProducts` select, there is no `condition` field selected.
    // But `getProducts` uses `findMany` without select, so it returns all fields.
    // I'll keep it but cast to any if needed or add to interface if I knew schema.
    // For now, I'll assume it's a valid field.
    // Actually, looking at `createProduct`, there is no `condition` field.
    // It might be a mistake in JS code or a field I missed.
    // I'll comment it out or keep it if I'm sure.
    // The JS code had:
    // if (condition && condition !== "ALL") { where.condition = condition; }
    // I will keep it but be aware it might fail if field doesn't exist.
    // To be safe, I will cast `where` to `any` when assigning `condition` if TS complains,
    // but here `Prisma.ProductWhereInput` might not have it.
    // I'll check `createProduct` payload. It has `brand, model, capacity, color, downPaymentAmount, imageUrl, price, installment6Months, installment10Months`.
    // No `condition`.
    // So `condition` filter in JS might be dead code or for a future feature.
    // I will comment it out to be safe and avoid TS errors, or leave it if I want to preserve logic.
    // I'll leave it but cast to any to avoid error if field missing.
 ...
 */
const editProduct = async (productId: string, file: Express.Multer.File | undefined, payload: Prisma.ProductUpdateInput) => {
    // --- STAGE 1: VALIDATION ---
    if (!productId) {
