...
const createInternalTransfer = catchAsync(async (req: Request, res: Response) => {
  ...
  const senderTransaction = await transactionService.createInternalTransfer(userId, {
    line_user_id,
    recipientUserId,
    amount,
    pin,
...
/**
 * คอนโทรลเลอร์สำหรับดึงประวัติธุรกรรมของ Wallet
 * @description รับ `walletId` จาก URL parameters และ options จาก Query String
 * @param {object} req - อ็อบเจกต์ Express Request
 * @param {object} res - อ็อบเจกต์ Express Response
 */
