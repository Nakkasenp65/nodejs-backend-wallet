 ...
 */
import { MissionType, TransactionStatus, NotificationType, Prisma } from "../../../generated/prisma/index";
 ...
 */
const getUserFirstTimeById = async (userId: number) => {
    const user = await prisma.user.findUnique({
        where: { id: userId },
 ...
 */
const createUserWithGoal = async (userData: any) => {
    const {
        mobileId,
        planId,
        line_user_id,
 ...
 */
const updateUserByAdmin = async (userId: number, payload: any) => {
 ...
 */
const createReferral = async (newcomerId: number, referralCode: string) => {
 ...
 * @returns {Promise<object>} Promise ที่ resolve เป็นอ็อบเจกต์ผู้ใช้ที่อัปเดตแล้ว
 */
const setUserReferCode = async (userId: number, referCode: string) => {
