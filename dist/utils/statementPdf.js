...
function createHtmlContent(data) {
    ...
    const totalIncome = validTransactions.reduce((sum, tx) => (tx.toWalletId === currentWalletId ? sum + tx.amount : sum), 0);
    const totalExpense = validTransactions.reduce((sum, tx) => (tx.fromWalletId === currentWalletId ? sum + tx.amount : sum), 0);
    ...
    return new Promise((resolve, reject) => {
        ...
        const options = {
            ...
            footer: {
                height: "20mm",
                contents: {
                    default: '<div style="font-family: NotoSansThai, sans-serif; font-size: 8px; text-align: center; color: #888;">หน้า {{page}}/{{pages}}<br>ขอขอบคุณที่ใช้บริการ NUMBER 1 MONEY PLUS</div>',
                },
            },
        };
...
//# sourceMappingURL=statementPdf.js.map