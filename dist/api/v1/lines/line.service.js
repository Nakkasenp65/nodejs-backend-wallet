...
const formatRecipientDisplay = (fullname, toString) => {
    ...
    if (!fullname || !toString || !toString.includes(" - ")) {
        console.warn(`Warning: Invalid input provided. Fullname: '${fullname}', To: '${toString}'. Returning fullname as fallback.`);
        return fullname;
    }
 ...
 * @returns {Promise<object>} Promise ที่ resolve เป็นข้อมูลการตอบกลับจาก LINE API
 */
const sendDepositFlexMessage = async (line_user_id, amount, balance, walletUniqueId, senderName, senderBankNumber, senderBankName, updatedDate) => {
 ...
 * @returns {Promise<object>} Promise ที่ resolve เป็นข้อมูลการตอบกลับจาก LINE API
 */
const sendWithdrawSuccessFlex = async (line_user_id, amount, balance, walletUniqueId, fullname, to, bank, updatedDate) => {
 ...
 */
const sendSenderFlex = async (senderLineId, sendingAmount, senderWalletUniqueId, receiverWalletUniqueId, updatedDate, senderBalance, liffUrlHistory) => {
    ...
    const flex = line.sendSender(senderLineId, senderWalletUniqueId, receiverWalletUniqueId, liffUrlHistory, formattedAmount, formattedDate, formattedBalance);
 ...
 * @returns {Promise<void>}
 */
const sendReceiverFlex = async (receiverLineId, receivingAmount, senderWalletUniqueId, receiverWalletUniqueId, updatedDate, receiverBalance, liffUrlHistory) => {
    ...
    const flex = line.sendReceiver(receiverLineId, senderWalletUniqueId, receiverWalletUniqueId, liffUrlHistory, formattedAmount, formattedDate, formattedBalance);
    await pushMessage(flex);
};
export default {
