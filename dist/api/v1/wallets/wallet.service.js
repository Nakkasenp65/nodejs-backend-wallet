 ...
 */
const getUserWallet = async (line_user_id) => {
    if (!line_user_id)
...
const updateWallet = async (walletId, updateBody) => {
    ...
    if (balance !== undefined) {
        ...
        if (isNaN(floatBalance))
            throw new ApiError(httpStatus.BAD_REQUEST, "Invalid balance amount.");
        dataToUpdate.balance = floatBalance;
    }
    if (bonusBalance !== undefined) {
        const floatBonus = parseFloat(bonusBalance);
        if (isNaN(floatBonus))
            throw new ApiError(httpStatus.BAD_REQUEST, "Invalid bonusBalance amount.");
