...
const updateTransaction = async (code, transactionId, verifyAmount = 0, senderName = "", sendBankName = "") => {
    ...
    if (transaction.status !== "PENDING") {
        console.warn(`Attempted to update an already processed transaction (ID: ${transactionId}, Status: ${transaction.status})`);
        return transaction;
    }
        ...
        case "200000": {
            ...
            const updatedTransaction = await prisma.$transaction(async (tx) => {
                ...
                if (isFirstTimeDeposit) {
                    ...
                    await createSuccessedTransaction(`คุณได้รับโบนัสเงินฝากครั้งแรก ${bonusAmount} บาท!`, bonusAmount, TransactionStatus.SUCCESS, "One Wallet", transaction.toWallet.user.line_display_name, `💵 พิเศษ! ออมครั้งแรก รับโบนัส 2 เท่า\n\nโบนัสเงินฝากครั้งแรก ${bonusAmount} บาท!`, transaction.wallet.id);
            ...
            try {
                ...
                if (successfulTxCount === 1) {
                    ...
                    if (referralRecord) {
                        ...
                        console.log(`[Referral Trigger] Newcomer was referred by ${referrerId}. Triggering mission update for referrer.`);
                        await userMissionService.checkAndUpdateMissionProgress(referrerId, "NEWCOMER_FIRST_DEPOSIT", { newcomerId: newcomerId });
                    }
                    else {
                        console.log(`[Referral Trigger] Newcomer ${newcomerId} was not referred. No referral mission update needed.`);
                    }
                }
            }
            catch (error) {
                console.error(`[Referral Trigger] Failed to process post-deposit referral check for TxID ${transactionId}:`, error);
            }
...
const createWithdrawTransaction = async (userId, amount, withdrawalDetails) => {
    ...
    const newWithdrawalTransaction = await prisma.$transaction(async (tx) => {
        ...
        console.log(`Withdraw successfully: on process to withdraw ${createdTransaction.amount}฿ - ${withdrawalDetails.bank}`);
...
const createInternalTransfer = async (senderUserId, transferData) => {
    ...
    await lineService.sendSenderFlex(flexData.senderLineId, flexData.sendingAmount, flexData.senderWalletUniqueId, flexData.receiverWalletUniqueId, flexData.updatedDate, flexData.senderBalance, flexData.liffUrlHistory);
    await lineService.sendReceiverFlex(flexData.receiverLineId, flexData.sendingAmount, flexData.senderWalletUniqueId, flexData.receiverWalletUniqueId, flexData.updatedDate, flexData.receiverBalance, flexData.liffUrlHistory);
...
const handleApproval = async (transactionId, existingTransaction, dataToUpdate) => {
    ...
    switch (existingTransaction.type) {
        ...
        case "WITHDRAW":
            ...
            break;
        default:
            throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, `Approval logic not implemented for type: ${existingTransaction.type}`);
    }
    if (!targetWalletId) {
        throw new ApiError(httpStatus.BAD_REQUEST, `Transaction type ${existingTransaction.type} is missing the required wallet association.`);
    }
    ...
    const updatedTransaction = await prisma.$transaction(async (tx) => {
        ...
        await notificationService.sendWithdrawSuccessNotification(updatedWallet.user.id, mainUpdatedTransaction.verifiedAmount, mainUpdatedTransaction.id);
        // userId, senderName, senderBankNumber, senderBankName, amount, updatedDate, to, line_user_id
        await lineService.sendWithdrawSuccessFlex(updatedWallet.user.line_user_id, mainUpdatedTransaction.verifiedAmount, updatedWallet.balance, updatedWallet.walletUniqueId, updatedWallet.user.fullname, mainUpdatedTransaction.to, mainUpdatedTransaction.bank, mainUpdatedTransaction.updatedAt);
...
const approveDeposit = async (transactionId, approvalData) => {
    ...
    const { mainUpdatedTransaction: updatedTransaction, flexData: flexData } = await prisma.$transaction(async (tx) => {
        ...
        if (isFirstTimeDeposit) {
            ...
            walletUpdateData.bonusBalance = { increment: bonusAmount };
            description += `\nคุณได้รับโบนัสเงินฝากครั้งแรก ${bonusAmount} บาท!`;
            await createSuccessedTransaction(`โบนัสเงินฝากครั้งแรก`, bonusAmount, TransactionStatus.SUCCESS, "SYSTEM_BONUS", transaction.toWallet.user.line_display_name, `โบนัสเงินฝากครั้งแรก ${bonusAmount} บาท`, walletId);
        }
    ...
    try {
        ...
        await notificationService.sendDepositSuccess(updatedTransaction.toWallet.userId, updatedTransaction.verifiedAmount, updatedTransaction.id);
        // ตรวจสอบภารกิจ
        ...
        await lineService.sendDepositFlexMessage(flexData.line_user_id, flexData.amount, flexData.balance, flexData.walletUniqueId, flexData.accountName, flexData.accountNumber, flexData.bankName, flexData.updatedDate);
    }
    catch (error) {
...
const rejectDeposit = async (transactionId, rejectionData) => {
    ...
    if (existingTransaction.status !== "PENDING") {
        console.warn(`Attempted to reject an already processed transaction (ID: ${transactionId}, Status: ${existingTransaction.status})`);
        return prisma.transaction.findUnique({ where: { id: transactionId } });
    }
    let description;
    switch (code) {
