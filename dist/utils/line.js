...
export const flexMessage = (mode = null, line_user_id, payload = {}) => {
    ...
    if (!LIFF_URL)
        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, "LIFF URL is not defined");
    if (mode === "register") {
        //เช็คข้อมูุลที่ใช้
    ...
    if (mode === "save") {
        const { amount, fullnameWithBankNumber, walletUniqueId, date, balance, bankImageUrl, bankName, liffHistoryUrl } = payload;
        if (!amount ||
    ...
    if (mode === "withdraw") {
        const { amount, walletUniqueId, toDisplay, bankImageUrl, bankName, date, balance, liffHistoryUrl } = payload;
        if (!amount || !walletUniqueId || !toDisplay || !bankImageUrl || !bankName || !date || !balance || !liffHistoryUrl)
            throw new ApiError(httpStatus.BAD_REQUEST, "Failed flex message");
        return {
    ...
    if (mode === "receiver") {
        const { senderWalletUniqueId, receiverWalletUniqueId, liffUrlHistory, formattedAmount, formattedDate, formattedBalance, } = payload;
        if (!senderWalletUniqueId ||
...
function sendRegister(line_user_id, walletUniqueId, fullname, phone, balance) {
    if (!walletUniqueId || !fullname || !phone || !balance)
        throw new ApiError(httpStatus.BAD_REQUEST, "Flex message content failed");
    return {
        to: line_user_id,
...
function sendSender(line_user_id, senderWalletUniqueId, receiverWalletUniqueId, liffUrlHistory, formattedAmount, formattedDate, formattedBalance) {
    if (!senderWalletUniqueId ||
        !receiverWalletUniqueId ||
        !liffUrlHistory ||
        !formattedAmount ||
...
function sendReceiver(line_user_id, senderWalletUniqueId, receiverWalletUniqueId, liffUrlHistory, formattedAmount, formattedDate, formattedBalance) {
    if (!senderWalletUniqueId ||
        !receiverWalletUniqueId ||
        !liffUrlHistory ||
        !formattedAmount ||
