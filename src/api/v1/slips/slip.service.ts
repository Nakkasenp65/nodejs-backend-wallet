 ...
 */
const verfifySlip = async (slipImageUrl: string, transactionId: string) => {
    ...
    try {
        ...
        // Call transactionService to reject the transaction if verification fails
        // Note: updateTransaction signature might need to be checked, assuming it takes (code, id, amount) or similar based on JS code usage:
        // await transactionService.updateTransaction(error.response?.data.code, transactionId, 0);
        // In JS: error.response?.data.code might be undefined if error is not from axios response
        const errorCode = error.response?.data?.code || "VERIFICATION_FAILED";
        await transactionService.updateTransaction(errorCode, transactionId, 0);
 ...
 */
const uploadSlip = async (fileObject: Express.Multer.File, identifier: string) => {
